...
<img width="782" height="567" alt="image" src="https://github.com/user-attachments/assets/a8d70ea6-2476-42b8-b0d5-911ae0cc7042" />
 ...
 - Ý tưởng cốt lõi của von Neumann: code và data được lưu chung trong một vùng nhớ, CPU đọc lệnh từ bộ nhớ, giải mã, rồi thực thi — đây là lý do vì sao các lỗi như buffer overflow có thể nguy hiểm: dữ liệu và lệnh nằm cùng một không gian địa chỉ, nên ghi đè dữ liệu có thể ảnh hưởng tới luồng thực thi.
...
=> Hiểu phân cấp này giúp giải thích lý do truy cập Register/Stack lại nhanh hơn nhiều so với việc truy cập Heap lớn hay đọc file trên disk.
...
# 2. Kiến trúc x86-64 & Register 
( x86-64 hay AMD64 là kiến trúc CPU 64-bit, mở rộng từ x86 32-bit )
### - Một số điểm cốt lõi:
...
- Data segment: nơi lưu global variables và static variables là biến đã được khởi tạo và có gái trị khác 0.
- BSS: nơi lưu global variables và static variables là biến chưa được khởi tạo hoặc có gái trị = 0.
- Heap: cấp phát vùng nhớ động bằng các lệnh như (malloc, new), sau khi sử dụng heap xong ta cần release bằng (free, delete) mỗi khi đc release heap sẽ xẹp xuống. Nếu quên release trong Heap nó sẽ phình ra dẫn tới Memory leak.
- Stack: là vùng nhớ đc cấp phát tự động, có cấu trúc LIFO(last in, first out) và chứa các function frame. 
